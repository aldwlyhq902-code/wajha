...
_SENT_STATUSES = ("sent", "replied", "interested", "negotiating", "customer")
# الحالات «المتجاوِبة» = من ردّ بأي شكل.
_REPLIED_STATUSES = ("replied", "interested", "negotiating", "customer")
# الحالات «المهتمّة».
_INTERESTED_STATUSES = ("interested", "negotiating")
# الحالة «عميل» (تحوّل ناجح).
_CUSTOMER_STATUSES = ("customer",)

# تسميات عربية للحالات الشائعة (لعرضها في اللوحة).
STATUS_LABELS = {
    "new":         "جديد",
    "queued":      "بالانتظار",
    "sent":        "أُرسل",
...
def _status_of(p: dict) -> str:
    """حالة مُطبَّعة (نصّ صغير دون فراغات على الأطراف)."""
    s = p.get("status")
